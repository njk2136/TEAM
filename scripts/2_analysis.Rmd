...
summary(m3t)
#mediation
m3 <- lmer(rating ~ QUIC_sum  + deprivation_sum + Threat_sum + (1 | ID), data=IADS_pleasing)
summary(m3m)
m3m <- lmer(rating ~ QUIC_sum + CHAOS_sum + deprivation_sum + Threat_sum  + CD.RISC_total + (1 | ID), data=IADS_pleasing)
## need to figure out what the correct treatment variable is
mediation <- mediate(m3,m3m, treat="Threat_sum",mediator = "CD.RISC_total",boot=TRUE)
summary(mediation)
...
summary(m1t)
all_scored$CD.RISC_total
m1t <- lm(MBEA_total ~  CHAOS_Adolescence_total, data=all_scored)
