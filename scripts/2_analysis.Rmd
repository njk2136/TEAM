...
cor.test(scored_data$threat_childhood_total, scored_data$threat_adolescence_total, na.rm=TRUE)
```
...
#H2
```{r}
m1 <- lm(MBEA_total ~ QUIC_sum + CHAOS_sum + deprivation_sum + Threat_sum, data=all_scored_allSMH)
summary(m1m)
...
plot_model(m1t, show.values = TRUE,vline.color = "lightgrey") +
  ggtitle("MBEA Scores")
all_scored_allSMH <- all_scored[complete.cases(all_scored$MH_caregiverbeliefs),]
m1m <- lm(MBEA_total ~ QUIC_sum + CHAOS_sum + deprivation_sum + Threat_sum + MH_caregiverbeliefs, data=all_scored_allSMH)
mediate <- mediate(m1, m1m, boot = TRUE, treat = "CHAOS_sum", mediator = "MH_caregiverbeliefs")
summary(mediate)
