...
m5 <- lmer(rating ~ QUIC_sum + CHAOS_sum + deprivation_sum + Threat_sum + (1 | ID)+ (1 | soundID), data=envi_displeasing)
summary(m5)

m6 <- lmer(rating ~ QUIC_sum + CHAOS_sum + deprivation_sum + Threat_sum + (1 | ID)+ (1 | soundID), data=envi_pleasing)
summary(m6)
```
...
cor.test(all_scored$BMRQ_musicseek, all_scored$QUIC_sum)
...
m2t <- lm(HUMS_adult_unhealthy ~ QUIC_Adolescence_total + QUIC_child_total, data=all_scored)
summary(m2t)

m1 <- lm(HUMS_adolescence_healthy ~ QUIC_sum + CHAOS_sum + deprivation_sum + Threat_sum, data=all_scored)
summary(m1)

m1t <- lm(HUMS_adolescence_healthy ~ QUIC_Adolescence_total + QUIC_child_total, data=all_scored)
summary(m1t)

m2 <- lm(HUMS_adult_healthy ~ QUIC_sum + CHAOS_sum + deprivation_sum + Threat_sum, data=all_scored)
summary(m2)

m2t <- lm(HUMS_adult_healthy ~ QUIC_Adolescence_total + QUIC_child_total, data=all_scored)
summary(m2t)
