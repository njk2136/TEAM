...
#update to use this f(x) for other multicollinearity checks
...
summary(m3t)
#mediation
m3m <- lmer(rating ~ QUIC_sum  + deprivation_sum + Threat_sum + PASS_total + (1 | ID) + (1 | soundID), data=IADS_pleasing)
summary(m3m)
## need to figure out what the correct treatment variable is
mediation <- mediate(m3,m3m, treat="Threat_sum",mediator = "PASS_total",boot=TRUE)
summary(mediation)
...
summary(m1)
m1 <- lm(MBEA_total ~ CHAOS_sum + deprivation_sum + Threat_sum, data=all_scored)
summary(m1)
#timing follow-up
...
#H3
```{r}
m1 <- lm(BMRQ_total ~ QUIC_sum  + deprivation_sum + Threat_sum, data=all_scored)
summary(m1)

m1m <- lm(BMRQ_total ~ QUIC_sum + CHAOS_sum + deprivation_sum + Threat_sum, data=all_scored)
summary(m1m)
## need to figure out what the correct treatment variable is
...
cor.test(all_scored$CHAOS_sum, all_scored$Threat_sum)

m1 <- lm(BMRQ_socialreward ~ unpredictability_total  + deprivation_sum + Threat_sum, data=all_scored)
summary(m1)

#HUMS - both adolescent + current
m1 <- lm(HUMS_adolescence_unhealthy ~ unpredictability_total + deprivation_sum + Threat_sum, data=all_scored)
summary(m1)
...
m2 <- lm(HUMS_adult_unhealthy ~ QUIC_sum  + deprivation_sum + Threat_sum, data=all_scored)
m2 <- lm(HUMS_adult_unhealthy ~ CHAOS_sum  + deprivation_sum + Threat_sum, data=all_scored)
summary(m2)
