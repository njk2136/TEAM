...
#HUMS ~ eBMRQ
```{r}
m1 <- lm(HUMS_adult_unhealthy ~ BMRQ_socialreward + BMRQ_emotion + BMRQ_moodreg + BMRQ_sensorimotor + BMRQ_musicseek + BMRQ_absorption + PASS_total + STAI_total, data=all_scored)
summary(m1)

m1 <- lm(scale(HUMS_adult_healthy) ~ scale(BMRQ_socialreward) + scale(BMRQ_emotion) + scale(BMRQ_moodreg) + scale(BMRQ_sensorimotor) + scale(BMRQ_musicseek) + scale(BMRQ_absorption) + PASS_total + STAI_total, data=all_scored)
summary(m1)
summary <- summary(m1)
