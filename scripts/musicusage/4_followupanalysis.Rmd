...
##step 1: total effect. IV --> DV
m1 <-lm(data=followup_scored, CD.RISC_total_scaled ~ CHAOS_sum_scaled)
summary(m1)


#step 2: IV --> mediator
m2 <-lm(data=followup_scored, AIMS_current_total_scaled ~  CHAOS_sum_scaled)
summary(m2)

#step 3: mediator --> IV
m3 <-lm(data=followup_scored, CD.RISC_total_scaled ~  CHAOS_sum_scaled + AIMS_current_total_scaled)
summary(m3)

#step 4: mediation
m1 <- lm(data=followup_scored, CD.RISC_total_scaled ~  CHAOS_sum_scaled)
m2 <- lm(data=followup_scored, CD.RISC_total_scaled ~  CHAOS_sum_scaled + AIMS_current_total_scaled)
seed = 111

mediate <- mediation::mediate(m1,m2, treat = "CHAOS_sum_scaled", mediator = "AIMS_current_total_scaled", dropobs = TRUE)
summary(mediate)

#TRYING HAYES MEDIATION
