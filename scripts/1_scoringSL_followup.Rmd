...
MET_key <- MET %>%
  dplyr::select(ANSWER, text, display, Response) %>%
  subset(Response != "AUDIO PLAY REQUESTED") %>%
  subset(Response != "") %>%
  subset(display !="Practice") %>%
  subset(display !="practice_ritmo") %>%
  dplyr::select(ANSWER, text, display) %>%
  distinct()

write.csv(MET_key, file='../data/MET_key.csv')
```

#BMRQ
```{r}
bmrq <- ogsurveyresponses %>%
  dplyr::select("Please.choose.the.most.appropriate.category....When.I.share.music.with.someone.I.feel.a.special.connection.with.that.person.":"Please.choose.the.most.appropriate.category....When.I.hear.a.tune.I.like.a.lot.I.can.t.help.tapping.or.moving.to.its.beat.", "PublicID")

absorption <- followup %>%
  dplyr::select('AIMS_current3_1', 'AIMS_current13_1', 'AIMS_current15_1', 'AIMS_current29_1', 'Q158')

bmrq <- merge(bmrq,absorption, by.x="PublicID", by.y = 'Q158')
bmrq$Q158 <- NULL
bmrq$X21..Mark..Completely.Agree..for.this.question....Completely.Disagree.Completely.Agree <- NULL
absorption$Q158 <- NULL


ogsurveyresponses <- merge(ogsurveyresponses, bmrq, by.x="PublicID", by.y = 'PublicID')
ogsurveyresponses$PublicID <- NULL
bmrq$PublicID <- NULL
...
absorption_cols = c('AIMS_current3_1', 'AIMS_current13_1', 'AIMS_current15_1', 'AIMS_current29_1')
...
ogsurveyresponses$BMRQ_absorption <- apply(bmrq[absorption_cols],1, sum)
```
...
```
#correcting gold-msi for original df
```{r}
...
write.csv(all,file='../data/SLfollowupscored.csv')
```
