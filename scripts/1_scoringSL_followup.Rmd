...
#read in data
```{r}
ogsurveyresponses <- read.csv(file='../data/SL_followup/survey_responsesOGrun.csv') #score PAS
MET <- read.csv(file='../data/SL_followup/data_exp_106990-v4_task-sv8h.csv') #clean & score MET
followup <- read.csv(file='../data/SL_followup/Groove)_February 25, 2023_12.54.csv') #score everything
ogBMRQ_goldMSI <- read.csv(file='../data/SL_followup/across_studies_bmrq_msi.csv')
```

#getting raw BMRQ
```{r}
ogBMRQ_goldMSI$bmrq_all <- ogBMRQ_goldMSI$bmrq_all * 20

ogBMRQ_goldMSI$bmrq_ms <- ogBMRQ_goldMSI$bmrq_ms * 4
ogBMRQ_goldMSI$bmrq_ee <- ogBMRQ_goldMSI$bmrq_ee * 4
ogBMRQ_goldMSI$bmrq_sm <- ogBMRQ_goldMSI$bmrq_sm * 4
ogBMRQ_goldMSI$bmrq_sw <- ogBMRQ_goldMSI$bmrq_sw * 4
ogBMRQ_goldMSI$bmrq_mr <- ogBMRQ_goldMSI$bmrq_mr * 4
```
...
deprivation_child <- followup %>%
  select("DT_child1_1":"DT_child25_1")
...
threat_child <- followup %>%
  select("DT_child26_1":"DT_child45_1")
...
threat_adolescence <- followup %>%
  select("DT_adol26_1":"DT_adol45_1")
...
HUMS_adolescent <- followup %>%
  select("HUMS_adol1_1":"HUMS_adol13_1")
...
#HUMS Scoring
HUMS_adult <- followup %>%
  select("HUMS_current1_1":"HUMS_current13_1")
...
CHAOS_Adolescence <- followup %>%
  select("CHAOS_Adolescence1_1":"CHAOS_Adolescence15_1")
...
```{r}
AIMS <- followup %>%
  select("AIMS_current1_1":"AIMS_current34_1")
...
escapism_current <- followup %>%
  select("Escapism_current1_1":"Escapism_current11_1")

cols_positive_escapism_current <- c("Escapism_current1_1","Escapism_current2_1","Escapism_current3_1","Escapism_current4_1","Escapism_current5_1","Escapism_current6_1")
  
cols_negative_escapism_current <- c("Escapism_current7_1","Escapism_current8_1","Escapism_current9_1","Escapism_current10_1","Escapism_current11_1")
...
escapism_adol <- followup %>%
  select("Escampism_adol1_1":"Escapism_adol11_1")

cols_positive_escapism_adol <- c("Escampism_adol1_1","Escapism_adol2_1","Escapism_adol3_1","Escapism_adol4_1","Escapism_adol5_1","Escapism_adol6_1")
  
cols_negative_escapism_adol <- c("Escapism_adol7_1","Escapism_adol8_1","Escapism_adol9_1","Escapism_adol10_1","Escapism_adol11_1")
...
SRQ_current <- followup %>%
  select("SRQ_current1_1":"SRQ_current8_1")
...
#adol
SRQ_adol <- followup %>%
  select("SQR_adol1_1":"SQR_adol8_1")
...
master <- merge(followup, ogBMRQ_goldMSI, by="id")
