...
#how are the subscales relating to one another?
variables = dplyr::select(second, 
                          MH_total,MH_caregiverbeliefs,
                          MH_caregiversinging, MH_childhoodattitude, MH_musicmaking, MH_socialcontexts)
output = corr.test(variables)
...
BMRQ_socialreward <-
ggplot(all, aes(x=MH_socialcontexts, y=BMRQ_socialreward)) + 
  theme_classic() +
  geom_point(position="jitter") + 
  geom_smooth(method=lm) + 
  xlab("M@HR Social Listening Contexts") +
...
m1 <- lm(scale(MH_socialcontexts) ~ scale(BMRQ_moodreg) + scale(BMRQ_emotion) + scale(BMRQ_socialreward) + scale(BMRQ_absorption) + scale(BMRQ_musicseek) + scale(BMRQ_sensorimotor), data=all)
summary(m1)
...
```{r}
mh <- all$MH_childhoodattitude
x <- all$BMRQ_musicseek
...
```{r}
#Test-Retest
testretest_table <- testretest_table %>%
    dplyr::rename_with(
      ~ case_when(
        . == "Music.Home...Retrospective.Subscales" ~ "Music@Home - Retrospective Subscales ",
        . == "Test.Retest..ICC..Over.4.Months" ~ "Test-Retest (ICC) Over 4 Months",
        TRUE ~ .
      )
    )

testretest_table_apa <-
  nice_table(
    testretest_table[1:8, ],
    title = c("Table 3", "Test-Retest Reliability"),
    note = c("Music@Home - Retrospective test-retest reliability measured via intra-class correlation (ICC) values between Study 1 and Study 2 (4 months delay) for 46 participants. The following thresholds were used: fair—0.40 ≤ values ≤ 0.59; good— 0.60 ≤ values ≤ 0.74; and excellent— values ≥ 0.75 for test-retest reliability.")
  )
print(testretest_table_apa)
flextable::save_as_docx(testretest_table_apa, path = '../../figures/m@h/testretest_table.docx')
  
#Gold-MSI
goldmsi_table <- goldmsi_table %>%
    dplyr::rename_with(
      ~ case_when(
        . == "Music.Home...Retrospective.Subscales" ~ " ",
        . == "Active.Engagement" ~ "Active Engagement",
        . == "Perceptual.Abilities" ~ "Perceptual Abilities",
        . == "Musical.Training" ~ "Musical Training",
        . == "Singing.Abilities" ~ "Singing Abilities",
        . == "Emotion" ~ "Emotion",
...
goldmsi_table_apa <-
  nice_table(
    goldmsi_table[1:7, ],
    title = c("Table 4", "Gold-MSI Subscale Associations"),
    note = c("Bivariate correlations relating Goldsmith Musical Sophistication Index (Gold-MSI) subscales (top row) and Music@Home - Retrospective subscales (left column) in the Study 2 sample. * p < .05, ** p <.01, *** p <.001.")
  )
print(goldmsi_table_apa)
flextable::save_as_docx(goldmsi_table_apa, path = '../../figures/m@h/goldmsi_table.docx')

#eBMRQ
ebmrq_table <- ebmrq_table %>%
    dplyr::rename_with(
      ~ case_when(
        . == "Music.Home...Retrospective.Subscales" ~ " ",
...
ebmrq_table_apa <-
  nice_table(
    ebmrq_table[1:8, ],
    title = c("Table 5", "eBMRQ Subscale Associations"),
    note = c("Bivariate correlations relating Extended Barcelona Music Reward Questionnaire subscales (eBMRQ) subscales (top row) and Music@Home - Retrospective subscales (left column) in the Study 2 sample. * p < .05, ** p <.01, *** p <.001.")
  )
print(ebmrq_table_apa)
flextable::save_as_docx(ebmrq_table_apa, path = '../../figures/m@h/ebmrq_table.docx')
...
print(SRQ_table_apa)
flextable::save_as_docx(SRQ_table_apa, path = '../../figures/m@h/SRQ_table.docx')
