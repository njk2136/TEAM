...
ggplot(all, aes(x=MH_childhoodattitude, y=STAI_total)) + 
  ...
  geom_point(position="jitter") + 
  geom_smooth(method=lm) + 
  xlab("M@HR Attitude Toward Childhood Music Environment") +
  ylab("STAI") +
...
ggplot(second, aes(x=MH_socialcontexts, y=BMRQ_socialreward)) + 
  ...
  geom_smooth(method=lm) + 
  xlab("M@HR Social Contexts") +
  ylab("BMRQ Social Reward") +
  theme(axis.title=element_text(size=20,face="bold"), axis.text=element_text(size=15))
print(BMRQ_socialreward)
ggsave("Figure2_A.png", width = 6.5, height = 4)
...
m1 <- lm(BMRQ_socialreward ~ MH_caregiverbeliefs + MH_caregiversinging + MH_childhoodattitude + MH_musicmaking + MH_socialcontexts, data = second)
summary(m1)

BMRQ_socialreward <- plot_model(m1, show.values = TRUE,vline.color = "lightgrey") + 
  ylim(-.5, .5) +
  ggtitle("BMRQ Social Reward") +
  ylab("Standardized Beta Coefficient Estimates") +
  theme(axis.title=element_text(size=17), axis.text=element_text(size=17))
print(BMRQ_socialreward)
ggsave("Figure2_C.png", width = 6.5, height = 4)

m1 <- lm(SRQ_current_total ~ MH_caregiverbeliefs + MH_caregiversinging + MH_childhoodattitude + MH_musicmaking + MH_socialcontexts, data = all)
summary(m1)

SRQ <- plot_model(m1, show.values = TRUE,vline.color = "lightgrey") + 
  ylim(-.5, .5) +
  ggtitle("SRQ") +
  ylab("Standardized Beta Coefficient Estimates") +
  theme(axis.title=element_text(size=17), axis.text=element_text(size=17))
print(SRQ)
ggsave("Figure2_D.png", width = 6.5, height = 4)
